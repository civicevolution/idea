(function($) {
  ...
  $.debug = function(variable, options) {
    ...
    function getObjectHtml(obj, depth){
			if(!depth) depth = 0;
      var html = "";
      var br   = "<br />";
      ...
      for (var prop in obj) {
        if(prop != "prototype"){
          ...
          if(obj[prop] instanceof Array && settings.recursive){
          ...
          else if(obj[prop] instanceof Function && settings.recursive&& settings.showFunction){
            html += doFunctionHtml(obj, prop);
          }
          else if(obj[prop] instanceof Object && settings.recursive){
									if(depth < settings.maxDepth ){
										html += doObjectHtml(obj, prop, ++depth);
									}else{
            ...
            html += "<span style=\"display:block;\">"+prop+": "+obj[prop]+"</span>";
						// html += "<span style=\"display:block;\"><a href=\"#\">"+prop+": "+obj[prop]+"</a></span>";
          }
        }
      }
      return html;
