...
function request_ape_send(){
 ...
 //console.log("request_ape_send for item: " + id)
	$.get('http://cgg.1civicevolution.org/team/send_ape/',{id: id}, 
...
function show_embedded_com_form(){
	try{
		//this.value = '';
		//$(this).css('color','black');
		$this = $(this);
		//var par = $this.closest('div.idea');
		//var tgt_id = Number(par.attr('id').match(/\d+/));
		var tgt_id = Number($this.attr('href').match(/\d+$/));
		if ($this.closest('td').children('div:first').hasClass('bs_idea')){
			var type = 'idea';
			var type_val = 11;
		}else{
			var type = 'answer';
		...
		//console.log("show_embedded_com_form for type: " + type)
		
		var tr = $this.closest('tr');
		var discussion_div = $this.closest('tr').next().find('div.embedded_discussion');	
		
		if(this.innerHTML.match(/close/i)){
		 //console.log("show_embedded_com_form close the form/discussion")
			discussion_div.closest('tr').remove();
			this.innerHTML = this.getAttribute('title_count')
			return false;			
		}
		this.setAttribute('title_count',this.innerHTML)
		this.innerHTML = 'Close discussion';
		
		//console.log("show_embedded_com_form id: " + tgt_id );
		var question = $this.closest('div.Question')
		var par_id = Number(question.attr('id').match(/\d+/));
		//console.log("show_embedded_com_form par_id: " + par_id)

		// create the discussion div
//		var par = $this.closest('td');
//		var discussion_div = $('div.embedded_discussion',par);
		var tr = $this.closest('tr');
		var discussion_div = $this.closest('tr').next().find('div.embedded_discussion');
		
		if( discussion_div.size() == 0){
			//discussion_div = $('<div class="embedded_discussion"><div class="inner_embedded_discussion"><p class="clear_both end"></p></div></div>');
			//par.append(discussion_div)
			discussion_div = $('<tr><td colspan="2"><div class="embedded_discussion"><div class="inner_embedded_discussion" id="target_discussion_' + type_val + '_' + tgt_id + '"><a href="#" class="open_all_idea_coms">Open all comments</a><p class="clear_both end"></p></div></div></td></tr>');
			tr.after(discussion_div)
		}

		// are there any comments targeted to this idea?
		//search under the parent question
		var embedded_comments = $('div.Comment_entry[target_id=' + tgt_id + '][target_type=' + type_val + ']').parent().clone(true);
		if(embedded_comments.size() > 0 ){
			embedded_comments.find('a.view_target').hide();
			//discussion_div.append( embedded_comments );
			$('p.end',discussion_div).before( embedded_comments )
			ellipsis(discussion_div);
			//var reply_link = $('<a href="#" class="idea_disc_reply">Add comment</a>');
			//reply_link.click(show_com_form);
			//$('p.end',discussion_div).before( reply_link )
		}
		var mode = 'add';
		var id = par_id;
		var resource_type = 'simple';
		//console.log("par_id: " + par_id)
		var form = $( jsonFn.add_comment_form({par_id : par_id, mode : mode, id : id, resource_type : resource_type, tgt_id: tgt_id, tgt_type: type_val }) );
		form.find('div.add_comment span.char_ctr:last').html(com_criteria + ' characters left');
		form.find('div.add_link span.char_ctr:last').html(res_criteria + ' characters left');
		form.find('div.attach_file span.char_ctr:last').html(res_criteria + ' characters left');
		$('div.add_comment label',form).html('Please comment on this ' + type);
		if(embedded_comments.size()==0)$('a.clear',form).removeClass('clear').addClass('cancel_embedded').html('Cancel');
		
		//discussion_div.append( form );
		$('p.end',discussion_div).before( form )
		//form.focus();
		//discussion_div.append( '<p class="clear_both"></p>')
		activate_comment_form(form);

	}catch(e){console.log("show_embedded_com_form error: " + e); }
	return false;
}

function activate_idea_forms(){
	$('form.add_bs_idea_form').each(
		function(){
			activate_idea_form( $(this) )
		})
}
	...
	$('button', form).unbind('click').click(
		function(){
		 //console.log("Submit the idea form");
			...
			form.ajaxSubmit({ 					
			  type: "POST", 
			  url: "/team/create_brainstorm_idea", 
				dataType: 'json',
...
function activateChatWindows(chat_container){
	...
	$('td.init_chat',chat_container).each( function(){this.innerHTML = '<p class="inst">Chat is now active. ' + 
		'Chat is for short conversations when teammates are online at the same time</p>' +
	...
	$(':button',chat_container).unbind('click').click( function(){
			try{
				chat_submit($(this.form));
			}catch(e){console.log("chat v1 error: " + e)}
			return false;
		}
	);
	$('form',chat_container).unbind('submit').submit(function() {
		try{
			chat_submit($(this));
		}catch(e){console.log("chat v2 error: " + e)}
...
function load_templates(templates){
	...
	$.getScript('/javascripts/ce/gen/templates.js',
		function(data){
			console.log("load templates")		
			$.get("/team/get_templates",
				function(data){ 
