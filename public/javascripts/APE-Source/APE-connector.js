...
var tmp = $.cookies.get('APE_Cookie', {'domain': document.domain}) || {'claimed_frequencies': {}	}; 

var claimed_freqs = tmp.claimed_frequencies;
// I only define subdomains 0-9, select one or report if none are available
var my_frequency = -1;
for(var i=0;i<10;i++){
	if(!claimed_freqs[i]){
		my_frequency = i;
		claimed_freqs[i] = 1
		console.log("my_frequency: " + my_frequency )
		config.frequency = my_frequency;
		$.cookies.set('APE_Cookie', {'claimed_frequencies': claimed_freqs	}, {'domain': document.domain});
		$(window).unload( function () { 
			// let go of this frequency
			//alert("Release frequecy: " + my_frequency +  ", bye now!"); 
			var tmp = $.cookies.get('APE_Cookie', {'domain': document.domain}) 
			var claimed_freqs = tmp.claimed_frequencies;
			delete claimed_freqs[my_frequency];
			$.cookies.set('APE_Cookie', {'claimed_frequencies': claimed_freqs	}, {'domain': document.domain});
		} );
		break;
	}
...
if(my_frequency == -1){
	console.log("There are no available frequencies")
	//$('<div><p>Sorry, we cannot connect to the live update server because you have too many connections aready.</p><p>Please close the browser and try again</p></div>').dialog( {title : 'Warning', modal : true } )
	return
}
