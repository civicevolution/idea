parent.console.log("XXX IFRAME: loading Core.js")
/*
...
APE.Core = new Class({
	...
	initialize: function(options){
		parent.console.log("XXX IFRAME: APE.Core.initialize")
		window.Ape = this;
		this.setOptions(options);

		this.selectTransport();
		this.request = new APE.Request(this);
		parent.console.log("XXX IFRAME: APE.Core.initialize 1")
		this.pipes = new $H; 
		this.users = new $H;
		...
		parent.console.log("XXX IFRAME: APE.Core.initialize 2")
		this.onRaw('login', this.rawLogin);
		this.onRaw('err', this.rawErr);
		this.onRaw('ident', this.rawIdent);
		this.onRaw('quit', this.rawQuit);
		parent.console.log("XXX IFRAME: APE.Core.initialize 3A")
		this.onError('003', this.clearSession);
		parent.console.log("XXX IFRAME: APE.Core.initialize 3B")
		this.onError('004', this.clearSession);
		parent.console.log("XXX IFRAME: APE.Core.initialize 3C")
		//Set core var for APE.Client instance
		if (options.init){
			this.ce_ver = '12.a'
			parent.console.log("XXX IFRAME: APE.Core.initialize 3D call options.init.apply(null, [this]);")
			parent.console.log("XXX IFRAME: this: " + this)
			parent.console.log("XXX IFRAME: this.ce_ver: " + this.ce_ver)
			parent.console.log("XXX IFRAME: [this]: " + [this])
			options.init.apply(null, [this]);
		} 
		parent.console.log("XXX IFRAME: APE.Core.initialize 4")
		//Execute complete function of APE.Client instance
		if (options.complete) options.complete.apply(null, [this]);
		this.fireEvent('load', this);
		parent.console.log("XXX IFRAME: APE.Core.initialize 5")
		if (this.options.connectOptions) this.start(this.options.connectOptions);
		parent.console.log("XXX IFRAME: APE.Core.initialize END")
	},
	...
	connect: function(args, options){
		...
		if (!$defined(options.sendStack) && options.sendStack !== false){
...
APE.init = function(config){
	//window.parent.console.log("inside APE.init")
	//Delay of 1ms allow browser to do not show a loading message
	(function() {
		parent.console.log("XXX IFRAME: inside APE.init")
		new APE.Core(config);
	}).delay(1);
}
