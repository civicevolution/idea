...
mobwrite.syncGateway = '/mob/mobwrite_conn';
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (name == 'F' || name == 'f') {
      ...
      // The server offers a compressed delta of changes to be applied.
			var emptyPatch = value.match(/^=\d+$/) ? true : false;
		  if (mobwrite.debug) {
		    window.console.info('name: ' + name + ', value: ' + value);
				window.console.info('emptyPatch: ' + emptyPatch );
		  }
			
      if (file && !emptyPatch ) {
			  if (mobwrite.debug) {
			    window.console.info('try to apply non empty patch: ' + value);
			  }
